...
module.exports = function(RED) {
    ...
    function TestModel(n) {
        RED.nodes.createNode(this,n);
        var node = this;

        this.modelfile = (n.modelfile || "").trim();
        this.trainsetcsv = null;
        ...
        this.on("input", function(msg) {
			...
			this.modelfile = this.modelfile.match(/(?:[^\s"]+|"[^"]*")+/g);
            this.modelfile = this.modelfile.shift(); // we take the first one, ignore everything if it has spaces...
            
            this.cmd_args = [ __dirname + "/TestModel.py", 
            				 "-p", __dirname,
            				 "-m", this.modelfile,
            				 "-d", this.trainsetcsv
            				 ];
    ...
    RED.nodes.registerType("Test-Model",TestModel);
}
